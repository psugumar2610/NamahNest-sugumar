(() => {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    if (readTimeElem) {
      readTimeElem.textContent = `\u{1F552} ${readTime} min read`;
    }
    ...
    const observerOptions = {
      ...
      // Trigger when heading is ~40% from top
      threshold: 0
    };
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderPagination(totalPages) {
      ...
      paginationControls.appendChild(createButton("\xAB Prev", currentPage - 1, currentPage === 1));
      for (let i = 1; i <= totalPages; i++) {
        paginationControls.appendChild(createButton(i, i));
      }
      paginationControls.appendChild(createButton("Next \xBB", currentPage + 1, currentPage === totalPages));
    }
